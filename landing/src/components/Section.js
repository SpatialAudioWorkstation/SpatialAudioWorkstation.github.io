...
function Section({ id, title, headerOnLeft, isVideo, children }) {
  return (
    <section id={id} className="py-2 md:py-10">
        <div>
            <div className={`flex flex-col ${headerOnLeft ? 'md:flex-row' : 'md:flex-row-reverse'} items-stretch`}>
                <div className={'w-full md:w-1/6 bg-section-bg-brown text-section-green flex justify-center items-center' +
                                            (headerOnLeft ? ' md:rounded-r-lg' : ' md:rounded-l-lg')}>
                    <h2 className="text-center text-xl md:text-2xl lg:text-4xl font-bold">{title}</h2>
                </div>
